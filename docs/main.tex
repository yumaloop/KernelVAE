...
\begin{document}
  ...
  \section{Deep Neural Networkのカーネル法による近似}
    \subsection{1層のFeed Forward Neural Network}
      ...
      \subsubsection{多層のNeural Network}
        ...
        \begin{align}
          ...
          =& \sigma_w^2 E_{{\bf z}^{(l-1)}} \left[ \sum_{j=1}^{N^{(l-1)}} \sum_{k=1}^{N^{(l-1)}} \phi(z_j^{(l-1)}) \phi(z_k^{(l-1)}) \right] \\
          =& \sigma_w^2 \sum_{i=1}^{N^{(l-1)}} E_{z_i^{(l-1)} \sim \mathcal{GP}(0, k^{(l-1)})} \left[ \phi^2( z_i^{(l-1)} )  \right] \\
          =& \sigma_w^2 N^{(l-1)} E_{z_i^{(l-1)} \sim \mathcal{GP}(0, k^{(l-1)})} \left[ \phi^2( z_i^{(l-1)} ) \right] 
        \end{align}
        となることに注意する．

...
flags.DEFINE_integer("num_train", 10000, "Number of training data.")
...
def do_eval(sess, model, x_data, y_data, save_pred=False):
    ...
    if save_pred:
        with tf.gfile.Open(
            os.path.join(FLAGS.experiment_dir, "gp_prediction_stats.npy"), "w"
        ) as f:
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.num_train <= 5000:
            ...
            tf.logging.info(
                ...
                % (min(FLAGS.num_train, FLAGS.num_eval), time.time() - start_time)
            )
        else:
            acc_train, mse_train, norm_train, final_eps = do_eval(
                sess, model, train_image[:1000], train_image[:1000]
            )
            tf.logging.info(
                "Evaluation of training set (%d examples) took %.3f secs"
        ...
        acc_valid, mse_valid, norm_valid, _ = do_eval(
            sess, model, valid_image[: FLAGS.num_eval], valid_image[: FLAGS.num_eval]
        )
        ...
        tf.logging.info(
            "Evaluation of test set (%d examples) took %.3f secs"
            % (FLAGS.num_eval, time.time() - start_time)
        )

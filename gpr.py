...
class GaussianProcessRegression(object):
  ...
  def _build_predict(self, n_test, full_cov=False):
    with tf.name_scope("build_predict"):
      self.x_test_pl = tf.identity(tf.placeholder(tf.float64, [n_test, self.input_dim], name="x_test_pl"))
  ...
  def _build_cholesky(self):
    ...
    self.k_data_data_reg = self.k_data_data + tf.eye(self.input_x.shape[0], dtype=tf.float64) * self.stability_eps
  ...
  def predict(self, test_x, sess, get_var=False):
    ...
    if self.l_np is None:
      ...
      self.k_np = sess.run(self.k_data_data, feed_dict={self.x_pl: self.input_x})

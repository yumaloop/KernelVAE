...
flags.DEFINE_boolean(
    "use_precomputed_grid", True, "Option to save/load pre-computed grid"
)
...
class NNGPKernel(object):
    ...
    """
        ...
        if FLAGS.use_precomputed_grid and tf.gfile.Exists(
            os.path.join(grid_path, grid_file_name)
        ):
            with tf.gfile.Open(os.path.join(grid_path, grid_file_name), "rb") as f:
                grid_data_np = np.load(f, encoding="latin1")
                tf.logging.info(
                    "Loaded interpolation grid from %s"
                    % os.path.join(grid_path, grid_file_name)
                )
                grid_data = (
                    tf.convert_to_tensor(grid_data_np[0], dtype=tf.float64),
    ...
    def get_var_fixed_point(self):
        with tf.name_scope("get_var_fixed_point"):
            ...
            current_qaa = (
                self.weight_var * tf.constant([1.0], dtype=tf.float64) + self.bias_var
            )
    ...
    """
        with tf.name_scope("Kdiag"):
            ...
            if self.use_fixed_point_norm:
                ...
                current_qaa = (
                    self.weight_var * tf.convert_to_tensor([1.0], dtype=tf.float64)
                    + self.bias_var
                )
    ...
    def k_full(self, input1, input2=None):
        ...
        with tf.name_scope("k_full"):
            cov_init = (
                tf.matmul(input1, input2, transpose_b=True) / input1.shape[1].value
            )
                        ...
                        with tf.name_scope("batch_%d" % b_x):
                            ...
                            for l in range(self.depth):
                                with tf.name_scope("layer_%d" % l):
                                    ...
                                    q_ab = self.weight_var * q_ab + self.bias_var
                                    corr_flat_batch = (
                                        q_ab / self.layer_qaa_dict[l + 1][0]
                                    )
    ...
    """
        ...
        batch_size = min(
            np.iinfo(np.int32).max // (FLAGS.fraction_of_int32 * input2_size),
            input1_size,
        )

...
class RBF:
    ...
    def set_params(self, new_params):
        ...
        self.alpha, self.gamma = (
            np.exp(new_params).copy().flatten()
...
class RBF_full:
    ...
    def gradients(self, x1):
        ...
        grads = [
            -g * np.square(diff[:, :, i]) * self.alpha * expdiff
...
class Linear:
    ...
    def set_params(self, new_params):
        assert new_params.size == self.nparams
        self.alpha, self.bias = np.exp(
            new_params
        ).flatten()  # try to unpack np array safely.
...
class Combined:
    ...
    def __call__(self, x1, x2):
        ...
        N2, D2 = x2.shape
        assert D1 == D2, "Vectors must be of matching dimension"
        return self.linear_kernel(x1[:, 0:1], x2[:, 0:1]) * self.RBF_kernel(
            x1[:, 1:], x2[:, 1:]
        )
